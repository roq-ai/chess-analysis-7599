...
import { deleteChessPositionById } from 'apiSdk/chess-positions';
...
function AccountViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AccountInterface>(
    ...
    () =>
      getAccountById(id, {
        relations: ['user', 'chess_position'],
      }),
  );

  const chess_positionHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('chess_position', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Chess Positions
                    </Text>
                    <NextLink passHref href={`/chess-positions/create?account_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.chess_position?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/chess-positions/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('chess_position', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/chess-positions/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('chess_position', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    chess_positionHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
